...
export default function ClientSidebar() {
  ...
  const avatar =
    avatarUrl ||
    (session?.user?.image?.startsWith('https://res.cloudinary.com')
      ? session.user.image
      : fallbackImage);
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex w-64 bg-white dark:bg-gray-900 h-screen border-r border-gray-200 dark:border-gray-700 px-4 py-6 flex-col items-start">
        <div className="flex flex-col items-center w-full mb-8">
          ...
          <button
            onClick={() => signOut()}
            className="mt-4 w-full px-4 py-2 text-sm border rounded-lg text-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700"
          >
        ...
        <nav className="space-y-2 w-full">
          {links.map(({ href, label, icon: Icon }) => {
            const isDashboard = href === '/client';
            const accessible = isDashboard || isEmailVerified;
